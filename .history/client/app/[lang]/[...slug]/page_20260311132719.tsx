...
export default async function page({params, searchParams}: pageProps) {
    ...
    const categoryData = categoriesWithIcons.find(c => c.name === search.)

    if(slug.length === 1 && categoriesWithIcons.find(c => c.name === slug[0])) {
        category = slug[0]
    } else if(slug.length === 2 && categoriesWithIcons.find(c => c.subcategories.find(s => s.name === slug[1]))) {
        category = slug[0]
        subCategory = slug[1]
    } else if (slug.length === 3 && categoriesWithIcons.find(c => c.subcategories.find(s => s.subcategories.find(ss => ss.name === slug[2])))) {
        category = slug[0]
        subCategory = slug[1]
