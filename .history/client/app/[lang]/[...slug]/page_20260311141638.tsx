...
export default async function page({params, searchParams}: pageProps) {
    ...
    const categoryObj = categoriesWithIcons.find(c => c.name === category);
    const subCategoryObj = categoryData?.subcategories
    
    const langCategory = categoryObj
    ? lang === 'ru'
        ? categoryObj.ru || categoryObj.name
        : categoryObj.uk || categoryObj.name
    : category;


  return (
    <div className='flex flex-col justify-center items-start w-full'>
        <h1 className='text-lg text-gray-500'>{langCategory && `${langCategory} |`} {subCategory && `${subCategory} |`} {subSubCategory && `${subSubCategory} |`} {city && `${city}`}</h1>
    </div>
  )
}
