...
export default async function page({params, searchParams}: pageProps) {
    ...
    if(cityData) {
        city = lang === 'ru' ? cityData.ru : cityData.uk || cityData.name
        path.pop() // убераем city что бы он дальше не искался как категория/подкатегория тд
    } 


    if (path.length === 1 && categoryData) {
        category = lang === 'ru' ? categoryData.ru : categoryData.uk || categoryData.name
    }
    ...
    if (sub) {
        category = lang === 'ru' ? categoryData.ru : categoryData.uk || categoryData.name
        subCategory = lang === 'ru' ? sub.ru : sub.uk || sub.name
    }
    }
    ...
    if (subSub && sub) {
        category = lang === 'ru' ? categoryData.ru : categoryData.uk || categoryData.name
        subCategory = category = lang === 'ru' ? sub.ru : sub.uk || sub.name
        subSubCategory = category = lang === 'ru' ? subSub.ru : subSub.uk || subSub.name
    }
}
