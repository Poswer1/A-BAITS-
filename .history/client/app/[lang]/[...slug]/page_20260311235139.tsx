...
export default async function page({params, searchParams}: pageProps) {
    ...
    if (categoryData) {
        ...
        if (sub) {
            subCategory = slug[1].toLowerCase()
        ...
        if (subSub) {
            subSubCategory = slug[2].toLowerCase()
  ...
  return (
    <div className='flex justify-start items-start w-full h-full pb-2 relative gap-2'>
        <Filter />
        <Catalog category={langCategory} subCategory={langSubCategory} subSubCategory={langSubSubCategory} city={langCity} lots={allLots}/>
    </div>
  )
}
