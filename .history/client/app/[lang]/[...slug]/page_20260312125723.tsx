...
export default async function page({params, searchParams}: pageProps) {
    ...
    try {
        allLots = await getFilterLot(category, subCategory, subSubCategory, city, search.minPrice, search.maxPrice, state, search.sort)
    } catch (error) {
        console.log('лотов нету', error)
        allLots = []
    }

    console.log(allLots.maxPriceLot || 'а хуй тебе')

  return (
    <div className='flex justify-start items-start w-full h-full pb-2 relative gap-2'>
        <Filter maxPriceLot={allLots.maxPriceLot}/>
        <Catalog category={langCategory} subCategory={langSubCategory} subSubCategory={langSubSubCategory} city={langCity} lots={allLots.lots} total={allLots.totalLot}/>
    </div>
  )
}
