...
export default async function page({params, searchParams}: pageProps) {
    ...
    try {
        allLots = await getFilterLot(category, subCategory, subSubCategory, city, search.minPrice, search.maxPrice, state, search.sort, searchValue)
    } catch (error) {
  ...
  return (
    <div className='flex justify-start items-start w-full h-full pb-2 relative gap-2'>
        ...
        <Catalog category={langCategory} subCategory={langSubCategory} subSubCategory={langSubSubCategory} city={langCity} lots={allLots.lots} total={allLots.totalLot} search={searchValue?.toString()}/>
    </div>
  )
}
