  ...
  return (
    <div className='flex flex-col justify-center items-center h-screen w-full text-black'>
        <div className='flex flex-col justify-center items-center sm:w-2/5 xl:w-2/6 2xl:w-1/4 rounded-md p-5 gap-5'>
            ...
            <h1 className='text-black text-center font-bold text-2xl md:text-2xl xl:text-3xl'>{t('auth', 'LoginInAccount')}</h1>
            ...
            <p className='w-full gap-1 text-gray-500'>{t('auth', 'DontHaveAccount')} <Link href={`/${lang}/auth/register`} className={`underline text-orange-600 ${hover}`}>Зареєструватись</Link></p>
        </div>
    </div>
  )
}
