  ...
  return (
    <div className='flex flex-col justify-center items-center h-screen w-full text-black'>
        <div className='flex flex-col justify-center items-center sm:w-2/5 xl:w-2/6 2xl:w-1/4 rounded-md p-5 gap-5'>
            ...
            <div className={customInput}>
                <input type={showConfirmPassword ? 'text': 'password'} placeholder={t('auth', '')} value={confirmPassword} onChange={(e) => setConfirPassword(e.target.value)} className={input}/>
                {showConfirmPassword ? (
            ...
            <p className='w-full gap-1 text-gray-500 text-center'>Вже зареєстровані? <Link href={`/${lang}/auth/login`} className={`underline text-orange-600 ${hover}`}>Увійти</Link ></p>
        </div>
    </div>
  )
}
