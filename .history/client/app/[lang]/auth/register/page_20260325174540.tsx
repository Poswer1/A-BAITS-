  ...
  return (
    <div className='flex flex-col justify-center items-center h-screen w-full text-black'>
        <div className='flex flex-col justify-center items-center sm:w-2/5 xl:w-2/6 2xl:w-1/4 rounded-md p-5 gap-5'>
            ...
            <h1 className='text-black text-center font-bold text-2xl md:text-2xl xl:text-3xl'>{t('auth', 'Register2')}</h1>     
            ...
            <button onClick={handleRegister} className={`bg-orange-600 text-white text-md p-2 flex justify-center items-center rounded-md ${hover} w-full`}>{t('auth','')}</button>
            <p className='text-sm text-center text-gray-500'>{t('auth', 'pesonalDate')}</p>
            <p className='w-full gap-1 text-gray-500 text-center'>{t('auth','alreadyRegistered')}<Link href={`/${lang}/auth/login`} className={`underline text-orange-600 ${hover}`}>Увійти</Link ></p>
        </div>
    </div>
  )
}
