    ...
    {loading ? (
      ...
      {confirmCreateOrder ? (
       <Success title={'Спасибо! Лот успешно создан и опубликован'}/>
        ...
        <>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center w-[90%] mt-10 gap-5 md:gap-0">
          <span onClick={handleBack} className={`${hover} md:hidden justify-center items-center`}><ChevronDown className="rotate-90"/> Назад</span>
          <h1 className="text-orange-600 text-3xl font-bold gap-2">{t('createLot','createLot-title')} <span className="text-black">лоту</span></h1>
          <span onClick={handleBack} className={`${hover} hidden md:flex justify-center items-center`}><ChevronDown className="rotate-90"/> Назад</span>
        </div>
