    ...
    <div className="flex flex-col justify-center items-center w-full relative min-h-150">
      ...
      {newBid && (
        <div className={`${overlay} flex-col gap-2`}>
          <AvatarBlock avatar={newBid?.avatar} size="80"/>
          <h1 className={`text-white text-xl font-bold ${animationScale}`}><span className="text-orange-600">{newBid?.name || 'Пользователь'} </span>{t('lot', 'userMadeNewBid')}</h1>
        </div>
      )}  
    </div>
  )
