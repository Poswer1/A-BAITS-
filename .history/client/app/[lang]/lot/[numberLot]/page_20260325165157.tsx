...
function page() {
  ...
  return (
    <div className="flex flex-col justify-center items-center w-full relative min-h-150 h-auto">
      {loading ? (
        <div className="flex flex-col md:flex-row justify-start items-start w-full 2xl:w-[80%] lg:w-[90%] py-2 md:gap-2 min-h-200 md:h-200">
          <div className="flex flex-col w-full md:w-1/2 gap-2">
            <div className="h-90 md:h-130 skeleton w-full"></div>
            <div className="flex md:justify-start justify-center flex-wrap w-full gap-4 md:gap-5">
              {Array.from({length: 8}).map((_, index) => (
                <div key={index} className="w-20 md:w-1/6 2xl:w-1/7 h-24 md:h-27 skeleton hidden md:flex"></div>
              ))}
              {Array.from({length: 4}).map((_, index) => (
                <div key={index} className="w-20 md:w-1/6 2xl:w-1/7 h-24 md:h-27 skeleton md:hidden"></div>
              ))}
            </div>
          </div>
          <div className="flex flex-col w-full md:w-1/4 gap-2">
            <div className="h-100 skeleton w-full"></div>
            <div className="h-57 skeleton w-full"></div>
          </div>
