...
async function page({params, searchParams}: pageProps) {
  ...
  try {
    date = await getReviewUser(name, Number(page))
  } catch (error) {
  ...
  return (
    <div className={`flex gap-5 flex-col w-full`}>
      <h1>Всего: {date.totalReview}</h1>
      {date?.allReview.map((review) => (
        <div className="flex flex-col justify-center items-start gap-2 bg-white rounded-md w-full">
      ...
      <Pagination total={date?.totalReview} maxLot={3}/>
