            ...
            <div className='flex flex-col justify-start items-start overflow-auto h-150 max-h-150 w-full mt-2 noScrollbar gap-2'>
              {typeChat === 'deal' && (
                <div className='w-full justify-center items-center text-center'>
                   <Image src={'/images/chat/deal.png'} alt='' width={200} height={200}/>
                   <h1>{t('chat', 'successDeal1')}</h1>
                   <p>{t('chat', 'successDeal2')}</p>
                   <div className='flex justify-center items-center gap-2'>
                    <Link href='' className={button}>{t('chat', 'ExchangeReview')}</Link>
                    <button className={`${button} text-black bg-transparent border border-ora`}>{t('chat', 'InviteModer')}</button>
                  </div>
                </div>
              )}
