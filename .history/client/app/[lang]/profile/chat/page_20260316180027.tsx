  ...
  return (
    <div className={`${pageContainerClass}`}>
      <div className='flex justify-between items-center w-full'>
        ...
        <h1 className={`flex justify-center 2xl:hidden gap-1 text-lg mb-5 ${selectChat ? 'block' : 'hidden'}`} onClick={() => setSelectChat('')}><X /> Закрыть чат</h1>
      </div>
      <div className='flex justify-center items-start gap-2'>
        ...
        <div className={`${blockClass} flex-col min-h-150 ${selectChat.length === 0 ? 'lg:hidden': ''}  lg:!w-200 2xl:!w-250 2xl:block !gap-0`}>
          {selectChat.length === 0 ? (
            ...
            <div className='flex flex-col justify-start items-start overflow-auto lg:h-100 2xl:h-150 2xl:max-h-150 w-full mt-2 noScrollbar gap-2'>
              {typeChat === 'deal' && (
            ...
            {status !== 'Close' ? (
              ...
              <h1 className={`text-xl flex justify-center items-center gap-1`}>{t('chat')}<Check className='text-orange-600'/></h1>
            )}
            </>
          )}
        </div>
