...
function page() {
  ...
  return (
    <div className={`${pageContainerClass} !h-100`}>
      <div className='flex justify-between items-center w-full'>
        <h1 className={`2xl:text-2xl lg:text-xl mb-5`}>Чат</h1>
        <h1 className={`${hover} flex justify-center 2xl:hidden gap-1 text-lg mb-5 ${selectChat ? 'block' : 'hidden'}`} onClick={() => setSelectChat('')}><X /> {t('chat', 'closeChat')}</h1>
      </div>
      <div className='flex justify-center items-start gap-2'>
        
        <ChatList setSelectChat={setSelectChat} setTypeChat={setTypeChat} selectChat={selectChat}/>

        <div className={`${blockClass} flex-col min-h-150 ${selectChat.length === 0 ? 'lg:hidden': ''}  lg:!w-200 2xl:!w-250 2xl:block !gap-0`}>
          {selectChat.length === 0 ? (
            <h1>чат не выбран</h1>
          ): (
            <>
            ...
            <div className='flex flex-col justify-start items-start overflow-auto lg:h-100 lg:h-max-100 2xl:h-150 2xl:max-h-150 w-full mt-2 noScrollbar gap-2'>
              {typeChat === 'deal' && (
              ...
              {messages?.map((msg, index) => {
                  ...
                  return (
                    ...
                    <div key={msg._id || index} className={`max-w-3/6 ${ isMyMessage ? 'self-end text-end': 'self-start text-start'}`}>
                      <p ref={messagesEndRef} className={`rounded-md p-2 ${ isMyMessage ? 'bg-orange-800/10 text-start' : 'bg-gray-100'}`}>
                        {msg?.message}
                      </p>
