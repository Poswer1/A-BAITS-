  ...
  return (
    <div className={`${pageContainerClass} h-150`}>
      ...
      <div className='flex justify-start items-start gap-2 w-full'>
        ...
        <div className={`${blockClass} flex-col min-h-150 xl:w-2/3 2xl:!w-3/5 !gap-0`}>
          {selectChat.length === 0 ? (
            <h1>чат не выбран</h1>
          ): (
            <>
            <div className='flex justify-between items-center w-full border-b border-b-gray-200 pb-2'>
              <Link href={`/${lang}/profile/${user?.name}`} className='flex gap-2'>
                <AvatarBlock avatar={lot?.images?.[0] ? lot?.images?.[0] : user?.avatar} size="50"/>
            ...
            {chat?.lot && (
              <Link href={`/${lang}/lot/${chat?.lot?.lot}`} className='flex justify-start items-start w-full border-b border-gray-300 p-2 gap-2 cursor-pointer'>
            ...
            <div ref={chatRef} className='flex flex-col justify-start items-start overflow-auto lg:h-110 lg:h-max-110 2xl:h-130 2xl:max-h-130 w-full mt-2 noScrollbar gap-2'>
              {chat?.type === 'deal' && (
