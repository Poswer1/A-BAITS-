...
function page() {
  ...
  return (
    <div className={`${pageContainerClass} !h-screen`}>
      <h1 className={`text-xl 2xl:text-2xl lg:text-xl mb-5`}>Чат</h1>
      <div className='flex flex-1 justify-start items-stretch gap-2 w-full overflow-hidden'>
        
        <ChatList setSelectChat={setSelectChat} setTypeChat={setTypeChat} selectChat={selectChat}/>

        <div className={`${blockClass} ${!selectChat ? 'hidden md:block': 'absolute top-0 left-0 md:static'} h-screen md:h-auto flex-col min-h-150 xl:w-2/3 2xl:!w-3/5 !gap-0 `}>
          {!selectChat ? (
            <h1>чат не выбран</h1>
          ): (
            <>
            ...
            <div ref={chatRef} className='flex flex-col justify-start items-start overflow-auto max-h-full md:h-110 md:h-max-110 2xl:h-130 2xl:max-h-130 w-full mt-2 noScrollbar gap-2'>
              {chat?.type === 'deal' && (
