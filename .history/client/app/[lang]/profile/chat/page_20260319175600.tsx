...
function page() {
  ...
  useEffect(() => {    
    ...
    socket.on('newReview', (data) => {
      setHaveReview(true)
      setMessages(prev => {
        const exist = prev.some(msg => msg._id === data._id)
        ...
        <div className={`${blockClass} ${!selectChat ? 'hidden md:block': 'fixed top-0 left-0 md:static'} h-screen md:h-160 2xl:h-190 flex-col xl:w-2/3 2xl:!w-3/5 !gap-0 `}>
          {!selectChat ? (
            ...
            <div ref={chatRef} className='flex flex-col justify-start items-start overflow-auto max-h-full h-full w-full mt-2 noScrollbar gap-2'>
              {chat?.type === 'deal' && (
                <div className={`w-full flex flex-col justify-center items-center text-center`}>
                   ...
                   <div className='flex justify-center items-center gap-2 mt-1'>
                    <Link href={`/${lang}/review/${selectChat}`} className={`${button} !p-1 md:!p-2`}>{t('chat', 'ExchangeReview')}</Link>
                    <button className={`${buttonWithoutBg} !p-1 md:!p-2`}>{t('chat', 'InviteModer')}</button>
                  </div>
                </div>
              )}
                    ...
                    return (
                      msg.to !== user?._id && (
                        <div className='flex justify-center items-center w-full'>
                          <div className={`flex flex-col justify-center items-center w-1/2 py-1 bg-orange-600/10 rounded-md`}>
              ...
              {/* {haveReview  && (
                <div className='flex justify-center items-center w-full'>
                  <div className={`flex flex-col justify-center items-center w-1/2 p-1 bg-orange-600/10 rounded-md`}>
                    <h1 className='text-base text-center'>{t('chat', 'NewReview')}</h1>
                  </div>
                </div>
              )} */}
