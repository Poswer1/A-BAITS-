    ...
    <div className='w-full flex flex-col justify-center items-center bg-gray-100 h-auto md:h-150 xl:h-200 text-black'>
        {loading ? (
            ...
            <div className='flex w-full flex-col justify-center items-center gap-2 bg-white rounded-xl p-2'>
                ...
                <div className='flex justify-start items-center w-full gap-2 rounded-lg '>
                    <img src={`${BASE_URL}${lot?.images?.[0]}`} className='w-15'/>
                    <div className='flex flex-col'>
                    <span className='md:hidden'>{lot?.name.length >= 30 ? lot?.name.slice(0, 30)+'...' : lot?.name}</span>
                    <span className='hidden md:flex'>{lot?.name}</span>
