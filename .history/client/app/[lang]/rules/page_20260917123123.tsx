      ...
      <article className="w-[90%] rounded-2xl bg-white m-5 md:m-10">
        ...
        <div className="mt-8 space-y-3 text-gray-700">
          {documentLines.map((line, index) => {
            ...
            if (line.startsWith('## ')) return <h2 key={line} className="pt-5 text-xl font-semibold text-gray-900">{line.slice(3)}</h2>
            if (line.startsWith('**')) return <p key={line} className="leading-7">{line.replace(/^\*\*|\*\*$/g, '')}</p>
            if (line.startsWith('- ')) return <li key={line} className="ml-5 list-item list-disc leading-7">{line.slice(2)}</li>
