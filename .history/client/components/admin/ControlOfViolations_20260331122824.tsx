...
export default function ControlOfViolations({allViolations}: ControlOfViolationsProps) {
    ...
    <div className='flex flex-col justify-start items-start gap-4 w-full'>
        ...
        <div className='flex flex-col justify-start items-start w-full'>
            {allViolations.map((v) => (
                <div key={v._id} className='flex justify-between items-center w-full bg-white border-t border-b border-gray-200 p-2'>
                    <div className='flex justify-start items-center gap-2'>
                        <AvatarBlock avatar={v.user.avatar} size='40'/>
                    </div>
                </div>
            ))}
        </div>
