            ...
            {allViolations.map((v) => (
                <div key={v._id} className='flex justify-between items-center w-full bg-white border-t border-b border-gray-200 p-2 h-20'>
                    ...
                    <div className='flex justify-start items-center gap-2'>
                        ...
                        <span className='text-sm w-30 text-gray-5'>Повтореные <br /> {v.repeated}</span>
                        <span className='text-sm w-30 text-gray-5'>Дата <br /> {getRelativeTime(v.createdAt, lang)}</span>
