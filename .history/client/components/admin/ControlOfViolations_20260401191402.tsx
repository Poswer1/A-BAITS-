...
export default function ControlOfViolations({allViolations}: ControlOfViolationsProps) {
    ...
    const handleChangeStatus = async (id:string, idV:string) => {
            try {
                const data = await changeStatus(id) 
                setListAllViolations(prev => prev.filter(v =>
                        v._id !== idV
                    ))
      ...
      const handleTemporary = async (id:string, idV:string) => {
        try {
            const day = Number(7)
            ...
            {listAllViolations.map((v) => (
                <div key={v._id} className={blockObj}>
                    ...
                    <div className='flex justify-start items-center gap-2'>
                        ...
                        <button onClick={() => handleChangeStatus(v.user._id, v._id)} className={`${button} bg-red-500 ml-10 !p-2`}>{t('admin', 'Blocked')}</button>
                        ...
                        <button onClick={() => handleTemporary(v.user._id, v._id)} className={`${button} bg-yellow-400 !p-2`}>{t('admin', 'TemporaryBlock')}</button>
                    </div>
                </div>
            ))}
        </div>
