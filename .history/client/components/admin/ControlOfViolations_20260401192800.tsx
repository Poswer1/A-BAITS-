            ...
            {listAllViolations.map((v) => (
                <div key={v._id} className={blockObj}>
                    ...
                    <div className='flex justify-start items-center gap-2'>
                        ...
                        <button onClick={() => handleTemporary(v.user)} className={`${button} bg-yellow-400 !p-2`}>{v.user.status === 'Temporary' ? t('admin', 'TemporaryUnBlock'): t('admin', 'TemporaryBlock')}</button>
                    </div>
                </div>
            ))}
        </div>
