        ...
        <div className='flex flex-col justify-start items-start w-full'>
            {listAllViolations.map((v) => (
                <div key={v._id} className={blockObj}>
                    ...
                    <div className='flex justify-start items-center gap-2'>
                        {(TemporarySelectTime && v.user.status === 'No restrictions') ? (
                            <>
                                <input 
                            ...
                            <>
                                ...
                                <button onClick={() => {setTemporarySelectTime(true), v.user.status !== 'No restrictions' ? handleTemporary(v.user._id) : setTemporarySelectTime(true)}} className={`${button} bg-yellow-400 !p-2`}>{v.user.status === 'Temporary' ? t('admin', 'TemporaryUnBlock'): t('admin', 'TemporaryBlock')}</button>
                            </>
                        )}
                    </div>
                </div>
