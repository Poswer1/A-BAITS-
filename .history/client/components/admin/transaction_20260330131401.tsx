    ...
    <div className="flex flex-col justify-start items-start gap-4">
        ...
        <div className="flex flex-col justify-start items-start w-full">
            {allTransactions.map(transaction => (
                <div key={transaction._id} className="flex justify-start items-center w-full bg-white border-t border-b border-gray-200 p-2">
                    <AvatarBlock avatar={transaction.user.avatar} size="40"/>
                    <h1 className="ml-2">{transaction.user.name}</h1>
                </div>
            ))}
        </div>
    </div>
