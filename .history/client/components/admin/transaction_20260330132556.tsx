            ...
            {allTransactions.map(transaction => (
                <div key={transaction._id} className="flex justify-between items-center w-full bg-white border-t border-b border-gray-200 p-2">
                    ...
                    <div className="flex justify-start items-center gap-4">
                        <span className="text-gray-500 text-sm text-center">Сумма: <br /><span className="text-black font-bold text-base">- {transaction.sum} ₴</span></span>
                        <span className="text-gray-500 text-sm text-center">Дата: <br /> <span className="text-black">{getRelativeTime(transaction.createdAt, 'ru')}</span></span>
                    </div>
                </div>
            ))}
        </div>
