      ...
      <div className="flex flex-col justify-start items-start gap-4">
        {listUser.map((user) => (
          <div className="flex justify-start items-center gap-2">
            ...
            <button className={`${user.status === 'Blocked' ? 'bg-white' : 'bg-red-500'} ${hover} p-2`} onClick={() => handleChangeStatus(user._id)}>{user.status === 'Blocked' ? t('admin', 'UnBlocked') : t('admin', 'Blocked')}</button>
          </div>
        ))}
      </div>
    </div>
