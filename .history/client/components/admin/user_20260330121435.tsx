          ...
          <div className="flex flex-col justify-start items-start">
            {filterLots.map((user) => (
              <div key={user._id} className="flex justify-between items-center gap-4 bg-white w-full p-2 border-t border-b border-gray-200">
                ...
                <div className='flex justify-start items-center gap-4'>
                  <button className={`${hover} ${user.role === 'Admin' ? 'bg-gray-500 cursor-not-allowed' : 'bg-blue-500'} p-2 text-white rounded-md`} disabled={user.role === 'Admin'} onClick={() => {setId(user._id), setEdit(true)}}>{t('admin', 'edit')}</button>
