...
export default  function User({listUser}: listUserProps) {
  ...
  const filterLots = listUser.filter(user =>
    ...
    <div className="flex flex-col w-full gap-2">
        ...
        {edit ? (
          ...
          <div className="flex flex-col justify-start items-start">
            {filterLots.map((user) => (
              <div key={user._id} className="flex justify-between items-center gap-4 bg-white w-full p-2 border-t border-b border-gray-200">
                <div className='flex justify-start items-center gap-4'>
                  ...
                  <span className='text-sm text-gray-500'>Баланс - {user.balance} ₴</span>
                </div>
                <div className={`justify-start items-center gap-4 ${balanceEdit ? 'hidden' : 'flex'}`}>
                  <button onClick={() => setBalanceEdit(prev => !prev)} className={`${button} !p-2`}>{t('admin', 'balance')}</button>
                ...
                <div className={`justify-start items-center gap-4 ${balanceEdit ? 'flex' : 'hidden'}`}>
                  <input type="number" className='outline-none border border-gray-300 rounded-md p-1' 
                  defaultValue={user.balance} 
                  value={newBalance}
                  onChange={(e) => setNewBalance(Number(e.target.value))} />
                  <button className={`${button} !p-2`}>{t('admin', 'saveBalance')}</button>
                  <X onClick={() => setBalanceEdit(false)}/>
                </div>
              </div>
            ))}
          </div>
