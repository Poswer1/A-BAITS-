      ...
      try {
        ...
        const data = await TemporaryBlock(id, day)
          setListUsers(prev => prev.map(u =>
                u._id === id ? {...u, status: data.status, UnblockDate: data.unBlockDate}
            ))
            ...
            {filterLots.map((user) => (
              ...
              <div key={user._id} className={blockObj}>
                ...
                <div className={`justify-start items-center gap-4 ${user._id === balanceEdit ? 'hidden' : 'flex'}`}>
                  ...
                  <button className={`${button} bg-yellow-400 !p-2`}>{user.status === 'Temporary' ? t('admin', 'TemporaryUnBlock'): t('admin', 'TemporaryBlock')}</button>
