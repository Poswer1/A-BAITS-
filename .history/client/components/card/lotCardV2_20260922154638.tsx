...
export default function LotCardV2({lot, select, selectLot}: LotCardV2Props) {
   ...
   const stateList = [
    ...
    const columnClass = `flex flex-col justify-center md:justify-start h-full items-start gap-1 w-full md:w-1/4 overflow-hidden`

  return (
    <Link 
    href={select === 'edit' ? `/${lang}/editLot/${lot.lotNumber}` : select ? '' : `/${lang}/lot/${lot.lotNumber}`} 
    onClick={() => (select && selectLot )? selectLot(lot._id) : ''} className={`cursor-pointer border-t border-b border-gray-200 flex justify-start items-start md:items-center gap-2 md:gap-10 bg-white w-full md:rounded-md text-base text-black overflow-hidden relative`}>
