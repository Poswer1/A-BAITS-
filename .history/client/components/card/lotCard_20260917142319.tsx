                ...
                <div className="flex flex-col justify-center items-start p-2 w-full bg-gray-100 gap-2 rounded-b-xl">
                    <h1 className="line-clamp-1">{lot.name}</h1>
                                        ...
                                        <span className="text-black text-sm md:text-base">
                                            <span className="hidden md:inline">{t('lot', 'lot-current-bid')}: </span><span className="text-xl font-bold text-orange-600"><br className="md:hidden" />{lot.startPrice} ₴</span>
                    </span>
                      ...
                      {t('lot', 'lot-location')}: <span className="font-bold text-black"><br/>{city || lot.location}</span>
                                        ...
                                        <div className="flex justify-start items-center gap-2 w-full">
                                                <button type="button" aria-label={opensChat ? t('lot', 'chat') : t('lot', 'lot-details-button')} onClick={opensChat ? openChat : undefined} className={`${hover} p-2 bg-orange-600 rounded-md text-white w-auto md:w-auto text-sm md:text-base`}>
                                                        {opensChat ? <MessageCircle className="md:hidden" size={18}/> : <Eye className="md:hidden" size={18}/>}<span className="hidden md:inline">{opensChat ? t('lot', 'chat') : t('lot', 'lot-details-button')}</span>
                        </button>
                                                 <FavoritesButton id={lot._id} compact/>
                    </div>
                </div>
            </div>
        </Link>
