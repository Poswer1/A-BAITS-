...
export default function TransactionCard({ transaction }: TransactionCardProps) {
    ...
    return (
   <div key={transaction._id} className="flex justify-between items-center w-full bg-white border-t border-b border-gray-200 p-2 min-h-20">
       <div className="flex justify-start items-center gap-2">
            ...
            <Link href={`/${lang}/lot/${transaction.lot.lotNumber}`}>
                ...
                <Link href={`/${lang}/profile/${transaction.user.name}`}>
                    <AvatarBlock avatar={transaction.user.avatar} size='40'/>
                </Link>
            )}
        ...
        <div className="flex justify-start items-center gap-10">
            <span className="text-gray-500 text-sm w-30">{transaction.type === 'Deposit' ? t('admin', 'deposit') : t('admin', 'withdrawal')}: <br /><span className="text-black font-bold text-base">{transaction.type === 'Deposit' ? '+' : '-'} {transaction.sum} ₴</span></span>
