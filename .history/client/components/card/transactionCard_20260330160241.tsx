   ...
   <div key={transaction._id} className="flex justify-between items-center w-full bg-white border-t border-b border-gray-200 p-2 h-20">
       <div className="flex justify-start items-center gap-2">
        ...
        <div className="flex justify-start items-center gap-10">
            <span className="text-gray-500 text-sm w-30">{transaction.type === 'Deposit' ? t('admin', 'deposit') : t('admin', 'withdrawal')}: <br /><span className={`${transaction.type === 'Deposit' ? 'text-green-400' : 'text-red-500'} font-bold text-base`}>{transaction.type === 'Deposit' ? '+' : '-'} {transaction.sum} ₴</span></span>
