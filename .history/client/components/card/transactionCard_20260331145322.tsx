...
export default function TransactionCard({ transaction, setTransactions }: TransactionCardProps) {
    ...
    return (
   <div key={transaction._id} className={`${blockO}`}>
       <div className="flex justify-start items-center gap-2">
        ...
        <div className="flex justify-start items-center gap-10">
            <span className={`text-gray-500 text-sm w-30`}>Статус: <br /> <span className='text-red-500'>{transaction.status || 'НЕТУ'}</span></span>
            <span className={`text-gray-500 text-sm w-30`}>{transaction.type === 'Deposit' ? t('admin', 'deposit') : t('admin', 'withdrawal')}: <br /><span className={`${transaction.status === 'Return' && 'opacity-50'} ${transaction.type === 'Deposit' ? 'text-green-500' : 'text-red-500'} font-bold text-base`}>{transaction.type === 'Deposit' ? '+' : '-'} {transaction.sum} ₴</span></span>
