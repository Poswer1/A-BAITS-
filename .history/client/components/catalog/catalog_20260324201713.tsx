...
export default function Catalog({category, subCategory, subSubCategory, city, lots, total, searchValue}:CatalogProps) {
  ...
  return (
    <div className='flex flex-col md:p-2 justify-start items-start w-full mt-2 md:min-h-200'>
        <h1 className='text-md text-gray-500 p-2'>{category && `${category} |`} {subCategory && `${subCategory} |`} {subSubCategory && `${subSubCategory} |`} {city && `${city}`}</h1>
        {searchValue && (
          <h1 className='text-black text-xl md:text-2xl p-2'>Результаты по поиску <span className='text-orange-600'>{`${searchValue.length >= 30 ? searchValue.slice(0, 30) + '...' : searchValue}`.toUpperCase()}</span></h1>
        )}
