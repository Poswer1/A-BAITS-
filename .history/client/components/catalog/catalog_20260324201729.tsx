...
export default function Catalog({category, subCategory, subSubCategory, city, lots, total, searchValue}:CatalogProps) {
  ...
  return (
    <div className='flex flex-col md:p-2 justify-start items-start w-full mt-2 md:min-h-200'>
        <h1 className='text-md text-gray-500 p-2 md:p-0'>{category && `${category} |`} {subCategory && `${subCategory} |`} {subSubCategory && `${subSubCategory} |`} {city && `${city}`}</h1>
        {searchValue && (
        ...
        <span className='text-gray-500 text-sm  p-2 md:p-0'>Найдено лотов: {total}</span>
