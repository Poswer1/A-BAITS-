  ...
  return (
    <div className='flex flex-col md:p-2 justify-start items-start w-full mt-2 md:min-h-200 overflow-x-hidden'>
        ...
        {activeFilters.length > 0 && (
          <div className="flex overflow-x-auto items-center gap-2 px-2 pt-2 md:px-0">
            {activeFilters.map(filter => (
              <button key={filter.key} type="button" onClick={() => removeFilter(filter.key)} className="flex items-center gap-1 rounded-md bg-orange-600/10 px-2 py-1 text-sm text-orange-700 hover:bg-orange-600/20">
                {filter.label}
                <X size={15} />
              </button>
            ))}
