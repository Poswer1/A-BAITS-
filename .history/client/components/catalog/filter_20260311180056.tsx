    ...
    <div className={`${open && 'w-90'} flex flex-col bg-white justify-start items-start h-full p-5 gap-4`}>
      ...
      <div className="flex flex-col justify-center items-center w-full">
        <div className="flex justify-center items-center gap-2 w-full">
          <InputField label={t('catalog', 'from')} onChange={Number(setMinPrice)} value={minPrice}/>
          <input min={0} value={minPrice} onChange={(e) => setMinPrice(Number(e.target.value))} className="w-full p-1 border border-gray-400 rounded-md"/>
          <input min={0} value={maxPrice} onChange={(e) => setMaxPrice(Number(e.target.value))} className="w-full p-1 border border-gray-400 rounded-md"/>
        </div>
      </div>
    </div>
  )
