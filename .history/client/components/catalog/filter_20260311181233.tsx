    ...
    <div className={`${open && 'w-90'} flex flex-col bg-white justify-start items-start h-full p-5 gap-4`}>
      ...
      <div className="flex flex-col justify-center items-center w-full gap-1">
        <div className="flex justify-center items-center gap-2 w-full">
          ...
          <InputField label={t('catalog', 'to')} type="number" onChange={setMaxPrice} value={minPrice}/>
        </div>
        <input 
        type="range"  
        min="0" 
        ...
        className="w-full bg-gray-100 border-none appearance-none rounded-xl h-3 slider"
        />
      </div>
    </div>
  )
