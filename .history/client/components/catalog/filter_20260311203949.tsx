...
export default function Filter() {
  ...
  return (
    <div className={`${open && 'w-120'} flex flex-col bg-white justify-start items-start h-full p-5 gap-4`}>
      ...
      <div className="flex flex-col justify-center items-center w-full gap-1">
        ...
        />
        <button className={`${button} w-full !p-1`}>{t('catalog', 'Apply')}</button>
      </div>
      ...
      <div className="flex flex-col justify-center items-start gap-2">
        ...
        <div className="flex flex-wrap justify-start items-center w-full gap-2">
          {stateList.map((state, index) => (
            <span key={index} className={`${hoverCat} py-1 px-2 rounded-md bg-gray-100`}>{state.name}</span>
          ))}
        </div>
      </div>
