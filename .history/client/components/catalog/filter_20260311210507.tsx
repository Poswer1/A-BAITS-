          ...
          <div className="flex flex-col justify-center items-start gap-2">
            ...
            <div className="flex flex-wrap justify-start items-center w-full gap-2">
              {stateList.map((s, index) => (
                <span key={index} onClick={() => handleAddStateToList(s.name)} className={`${hoverCat} ${state.includes(s.name) && 'bg-orange-600/20'} py-1 px-2 rounded-md bg-gray-100`}>{s.name}</span>
              ))}
            </div>
          </div>
        </div>
