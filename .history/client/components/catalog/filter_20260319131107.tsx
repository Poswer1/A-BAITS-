...
export default function Filter({maxPriceLot}: FilterProps) {
  ...
  return window.innerWidth >= 768; // >=768px — desktop открыто, <768 — закрыто
})
  ...
  return (
    <>
    <div className="bg-orange-600 md:hidden fixed top-1/2 p-1 flex rounded-r-2xl" onClick={() => setOpen(true)}>
      <Sliders className='text-white cursor-pointer' size={35}/>
    </div>
    <div className={`${open ? 'w-full top-0 fixed md:static md:w-100 p-5' : 'w-0 md:w-20'} md:p-5 transition-all duration-300 ease-in-out flex flex-col bg-white justify-start items-start h-screen gap-4 border-r border-gray-300 text-black`}>
