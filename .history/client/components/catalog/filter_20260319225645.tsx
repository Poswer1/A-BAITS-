...
export default function Filter({maxPriceLot}: FilterProps) {
  ...
  return (
    <>
    <div className={`${isOpen ? 'opacity-0' : 'opacity-100'} bg-orange-600 p-1 text-white fixed top-1/2 md:hidden rounded-r-2xl`} onClick={() => setOpenMobile(true)}>
      <Sliders size={35}/>
    </div>
    <div className={`${isOpen ? 'w-full top-0 fixed md:static md:w-100 p-5 z-20' : 'w-0 md:w-20'} md:p-5 transition-all duration-300 ease-in-out flex flex-col bg-white justify-start items-start h-screen gap-4 border-r border-gray-300 text-black`}>
