...
export default function LotForm({mode, initialData}:LotFormProps) {
      ...
      if(description.length > 1200) {
        ...
        try {
          if(mode === 'create') {
           ...
           handleClear()
          } else if(mode === 'edit' && initialData?.lotNumber){
  ...
  <div className="flex flex-col justify-center items-center w-full gap-5 md:gap-10 md:min-h-120 xl:min-h-200 bg-white md:bg-gray-100 text-black">
    {loading ? (
      ...
      <>
      {initialData?.historyBid.length > 0 ? (
        <div className="flex flex-col justify-center items-center">
          <X size=} className="text-red-500"/>
          <p className="text-red-500">{t('createLot', 'createLot-haveHistoryBid')}</p>
        </div>
      ) : (
      confirmCreateOrder ? (
