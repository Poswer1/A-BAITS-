...
import { block,input, nameInput, selectBlock } from "@/styles/createLot"
...
export default function MainInfoSections({name, setName, openCategory, setOpenCategory, category, subCategory, subSubCategory} : MainInfoProps) {
    ...
    const input = 'p-2 flex justify-start items-center outline-none rounded-md border-gray-100 w-full'
    const selectBlock = 'flex justify-between items-center w-1/2 bg-gray-100 p-2 rounded-md'
    const openBlock = 'flex flex-col justify-start items-start p-2 bg-gray-100 rounded-md absolute top-[100%] max-h-40 overflow-y-auto gap-2 custom-scrollbar'
