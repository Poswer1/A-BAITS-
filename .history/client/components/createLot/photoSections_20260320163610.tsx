          ...
          <div className="flex flex-wrap justify-start items-center w-full gap-3">
           <input type="file" multiple id="fileInput" onChange={handleFile} className="hidden"/>
            {Array.from({ length: 8 }).map((_, index) => (
              <label htmlFor="fileInput" key={index} className="flex justify-center items-center bg-gray-100 h-32 sm:w-2/5 md:w-1/5 cursor-pointer relative overflow-hidden">
                {preview[index] ? (
                  <>
