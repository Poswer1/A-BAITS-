...
export default function PhotoSections({setFile, file, preview, setPreview}:PhotoSectionsProps) {
    ...
    const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if(exestingWebp) {
        setMessage(t('createLot', 'formatFile'))
        setTimeout(() => {
          setMessage('')
        }, 3000)
        return
  ...
  return (
    <div className={block}>
       <div className={Blockinput}>
          ...
          <label htmlFor="fileInput" className={`${file.length > 0 && 'hidden'} md:hidden flex flex-col gap-2 justify-center items-center h-50 bg-orange-600/10 w-full rounded-lg`}>
            <Image className="text-orange-600/50" size={100}/>
            <h1 className="text-xl">Добавить фото</h1>
          </label>
          <div className={`${file.length > 0 ? 'flex' : 'hidden md:flex'} flex flex-wrap justify-center items-center w-full gap-3`}>
           <input type="file" multiple id="fileInput" onChange={handleFile} className="hidden"/>
            {Array.from({ length: 8 }).map((_, index) => (
              <label htmlFor="fileInput" key={index} className="flex justify-center items-center bg-gray-100 h-32 w-[45%] md:w-1/5 cursor-pointer relative overflow-hidden">
                {preview[index] ? (
                  <>
