                ...
                {preview[index] ? (
                  <>
                    <div onDrop={() => handleDrop(index)} draggable  onDragStart={() => setDragIndex(index)}  onDragOver={(e) => e.preventDefault()} className="w-full h-full">
                      <img src={initialPreview.length ? `${BASE_URL}${preview }` preview[index]} className="object-cover w-full h-full object-center rounded" onMouseEnter={() => setHoverFile(index)}/>
                    </div>
                    {hoverFile === index && (
