...
export default function PhotoSections({image, setImage, setFile, file, preview, setPreview, initialPreview}:PhotoSectionsProps) {
    ...
    const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setImage(prev => [...prev, ...newPreview])
    }

    const handleDeleteFile = (indexFile:number) => {
      setImage(prev => prev.filter((file, index) => index !== indexFile))
      setImage(prev => prev.filter((file, index) => index !== indexFile))
    }
    ...
    const handleDrop = (dropIndex: number) => {
      ...
      const updated = [...image] // новый массив
      const updatedFile = [...image] // новый массив

      handleDropAction(dropIndex, dragIndex, updated, setImage)
      handleDropAction(dropIndex, dragIndex, updatedFile, setImage)
    }
                ...
                {image[index] ? (
                  <>
                    <div onDrop={() => handleDrop(index)} draggable  onDragStart={() => setDragIndex(index)}  onDragOver={(e) => e.preventDefault()} className="w-full h-full">
                      <img src={initialPreview.length ? `${BASE_URL}${image.preview[index]}` : image?.preview[index]} className="object-cover w-full h-full object-center rounded" onMouseEnter={() => setHoverFile(index)}/>
                    </div>
                    {hoverFile === index && (
