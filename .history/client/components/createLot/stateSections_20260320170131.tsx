        ...
        <div className={Blockinput}>
            ...
            <div className="flex overflow-x-auto w-full justify-start items-center gap-5">
                {statesLot.map((state, index) => (
                    <>
                    <span key={index} className={`white ${hover} ${index === 3 && mode !=='state' && 'hidden'} ${stateLot === state.state && 'bg-orange-600 text-white'} bg-gray-100 p-2 w-40 text-center rounded-md`} onClick={() => setStateLot(state.state)}>{state.state}</span>
                    </>
                 ))}
            </div>
        </div>
