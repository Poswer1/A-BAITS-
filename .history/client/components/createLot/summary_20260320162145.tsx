...
export default function Summary({reservPrice, autoReExtension, advertising, handleCreateOrUpdate, message} : Summary) {
  ...
  return (
    <div className={`${block} gap-1`}>
      ...
      {autoReExtension && (
        <span className={`text-gray-500 text-sm ${animationOpacity}`}>{t('createLot','create-autoReExtension')} - <span className="text-ora">10 ₴</span></span>
      )}
      {advertising && (
        <span className={`text-gray-500 text-sm ${animationOpacity}`}>Реклама - 20 ₴</span>
      )}
      {reservPrice > 0 && (
        <span className={`text-gray-500 text-sm ${animationOpacity}`}>{t('createLot','createLot-ReservePrice')} - 20 ₴</span>
      )}
