                      ...
                      <ul className={`${listClass} border-l border-gray-400`}>
                          {sub.subcategories.map((subSub) => (
                            <li className={`${listClass}  ${animationOpacity} ml-6`}key={subSub.name}>
                              <span className={`${linkClass} ${hoverSub} text-gray-500`} onClick={() => setOpenCategory(false)} onMouseEnter={() => setSubSubCategory(prev => prev === subSub.name ? null : subSub.name)}>{}</span>
                            </li>
                        ))}
                      </ul>
                      )}
