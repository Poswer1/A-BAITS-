          ...
          <div onClick={(e) => e.stopPropagation()} className={`w-2/3 bg-gray-100 rounded-md ${openFrom === 'header' && 'top-[8%] absolute' }  flex justify-start items-start z-30`}>
            ...
            <ul className='flex flex-col justify-start items-start gap-3 border-r p-2'>
                {categoriesWithIcons.map((cat) => ( 
                  <li key={cat.name} onClick={hadleClick} onMouseEnter={() => handleSelectCat(cat.name)} className={`${hoverCat} ${openSubCategory === cat.name ? 'bg-orange-800/10 text-orange-600' : '' } flex justify-between p-1 rounded-md items-center w-full ${animationOpacity}`} >
              ...
              <ul className={`${listClass} p-2 gap-5`}>
                {activeCategory.subcategories.map((sub) => (
                  <li key={sub.name} className={`${listClass}  ${animationOpacity} ml-2 `} >
                    <span className={`${linkClass} ${hover} text-black`} onClick={hadleClick} onMouseEnter={() => handleSelectSub(sub.name, sub.subcategories.length > 0)}>
                      ...
                      <ul className={`${listClass} border-l border-gray-400`}>
                          {sub.subcategories.map((subSub) => (
                            <li className={`${listClass}  ${animationOpacity} ml-6`}key={subSub.name}>
                              <span className={`${linkClass} ${hoverSub} text-gray-500`} onClick={() => setOpenCategory(false)} onMouseEnter={() => setSubSubCategory(prev => prev === subSub.name ? null : subSub.name)}>{nameLang(subSub)}</span>
                            </li>
                        ))}
                      </ul>
                      )}
