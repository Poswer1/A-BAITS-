          ...
          <div onClick={(e) => e.stopPropagation()} className={`w-2/3 bg-gray-100 rounded-md ${openFrom === 'header' && 'top-[8%] absolute' }  flex justify-start items-start z-30`}>
            ...
            <ul className='flex flex-col justify-start items-start gap-3 border-r p-2'>
                {categoriesWithIcons.map((cat) => ( 
                  <li key={cat.name} onClick={handleClick} onMouseEnter={() => handleSelectCat(cat: cat.name)} className={`${hoverCat} ${openSubCategory === cat.name ? 'bg-orange-800/10 text-orange-600' : '' } flex justify-between p-1 rounded-md items-center w-full ${animationOpacity}`} >
