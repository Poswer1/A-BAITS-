...
import { ChevronDown, Fish, Zap, Activity, Settings, Link2,  Layers, Snowflake, ArrowRightCircle, Feather, Gift, Monitor, Box, X} from "lucide-react";
...
const nameLang = (item: { ru?: string; uk?: string; name: string }) =>
  ...
  return (
    ...
    <div className={`${overlay}`} onClick={() => setOpenCategory(false)}>
          <div onClick={(e) => e.stopPropagation()} className={`w-full md:w-2/3 bg-gray-100 rounded-md ${openFrom === 'header' && 'top-0 md:top-[8%] absolute' }  flex flex-col md:flex-row justify-start items-start z-30 text-black`}>
            
          <div className="flex justify-between items-center w-full text-black border-b py-2 md:hidden">
                <h1 className="text-black text-xl font-bold">{category || 'Категория'}</h1>
                <X />
            </div>
              
            <ul className={`${activeCategory ? 'hidden md' : ''} flex flex-col justify-start items-start gap-3 md:border-r p-2 w-full md:w-auto overflow-auto h-screen md:h-auto`}>
            ...
            {activeCategory && (
              <ul className={`${listClass} p-2 gap-5 md:static  bg-white justify-start md:bg-transparent h-full w-full md:w-auto`}>
