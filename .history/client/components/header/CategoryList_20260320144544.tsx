...
function CategoryList({setOpenCategory, openFrom, open} : CategoryList) {
  ...
  return (
    ...
    <div className={`${overlay}`} onClick={() => setOpenCategory(false)}>
          <div onClick={(e) => e.stopPropagation()} className={`${open ? 'w-full' : ''} w-full md:w-2/3 bg-white rounded-md ${openFrom === 'header' && 'top-0 md:top-[8%] absolute' }  flex flex-col md:flex-row justify-start items-start z-30 text-black`}>
