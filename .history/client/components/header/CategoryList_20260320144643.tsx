...
import { animate, overlay } from "@/styles/global";
...
function CategoryList({setOpenCategory, openFrom, open} : CategoryList) {
  ...
  return (
    ...
    <div className={`${overlay}`} onClick={() => setOpenCategory(false)}>
          <div onClick={(e) => e.stopPropagation()} className={`${open ? 'w-full' : 'w-0'}${animate}  bg-white rounded-md ${openFrom === 'header' && 'top-0 md:top-[8%] absolute' }  flex flex-col md:flex-row justify-start items-start z-30 text-black`}>
          ...
          <div className="flex justify-between items-center w-full text-black border-b border-gray-300 p-2 md:hidden bg-white">
            ...
            <ul className={`${(activeCategory || !open) && 'hidden md:flex'} flex flex-col justify-start items-start gap-3 md:border-r p-2 w-full md:w-auto overflow-auto h-screen md:h-auto`}>
