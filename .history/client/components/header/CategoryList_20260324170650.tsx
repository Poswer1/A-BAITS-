...
function CategoryList({setOpenCategory, openFrom, createLotCategory, createLotSetCategory, createLotSetSubCategory, createLotSetSubSubCategory, createLotSubCategory, createLotSubSubCategory} : CategoryList) {
  ...
  const handleClick = () => {
    ...
    if(openFrom === 'header') {
      ...
      if(category)createLotCategory(category)
      if(subCategory)setSubCategory(subCategory)
      if(subSubCategory)setSubSubCategory(subSubCategory)
    }
  ...
  return (
    ...
    <div className={`${overlay}`} onClick={() => setOpenCategory(false)}>
          <div onClick={(e) => e.stopPropagation()} className={`${animationOpacity} w-full md:w-2/3 bg-white rounded-md ${openFrom === 'header' && 'md:top-[8%] absolute' }  flex flex-col md:flex-row justify-start items-start z-30 text-black`}>
