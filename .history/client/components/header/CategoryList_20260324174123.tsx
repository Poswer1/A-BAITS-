...
function CategoryList({setOpenCategory, openFrom, createLotSetCategory, createLotSetSubCategory, createLotSetSubSubCategory} : CategoryList) {
  ...
  return (
    ...
    <div className={`${overlay}`} onClick={() => setOpenCategory(false)}>
      <div onClick={(e) => e.stopPropagation()} className={`${animationOpacity} w-full md:w-2/3 md:absolute ${openFrom === 'header' ? 'top-15' : ''} bg-white rounded-md flex flex-col md:flex-row justify-start items-start z-30 text-black h-full md:h-auto`}>
