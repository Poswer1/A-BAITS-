            ...
            <div className='flex justify-between items-center w-full p-2'>
                <Link href={`/${lang}`}>
                    <Image 
                        ...
                        width={200}
                        height={200}
                        className={`w-[100px] md:w-[150px] h-auto ${openSearch ? 'hidden md:block' : 'block'}`}
                    />
                </Link>
                <div className={`flex justify-center items-center gap-3 w-full`}>
                    <button onClick={() => setOpenCategory(prev => !prev)} className={`font-medium hidden md:block md:flex justify-center items-center gap-1 ${hover} bg-orange-600 text-white p-2 px-4 rounded-md`}><Menu size={20}/>{t('header','category')}</button>
                    <div className={`flex flex-col justify-center items-center w-full md:w-[60%] 2xl:w-[40%] relative ${openSearch ? 'block' : 'hidden md:block'}`}>
                ...
                <div className='flex justify-center items-center gap-5 whitespace-nowrap relative'>
                        ...
                        {openSearch ? (
                        <X className='text-black' onClick={() => setOpenSearch(false)}/>
                        ): (
                         <Search className='text-gray-500' onClick={() => setOpenSearch(prev => !prev)}/>
                        )}
