...
function OpenProfile({setOpenProfile, name, open}: OpenProfile) {
  ...
  return ( 
    <div ref={modalRef} className={`${open ? "h-full" : "h-0"} ${animate} flex flex-col justify-center items-center bg-white rounded-xl absolute right-[-10] md:right-0 top-10 text-black z-20 w-40`}>
        <div className={`а-г ${open ? 'flex opacity-100' : 'opacity-0'} flex flex-col`}>
