...
function OpenProfile({setOpenProfile, name, open}: OpenProfile) {
  ...
  return ( 
    <div ref={modalRef} className={`${open ? "h-70 md:h-60" : "h-0"} ${animate} overflow-hidden flex flex-col justify-center items-center bg-white rounded-xl absolute right-[-10] md:right-0 top-10 text-black z-20 w-40`}>
          ...
          <Link href={`/${lang}/profile/buy/`} className={linkClass} onClick={() => setOpenProfile(false)}>{t('header',"modalProfile-history")}</Link>
          <span className={`${linkClass} md:hidden`}>{t('header', 'notification')}</span>
