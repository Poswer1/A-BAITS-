...
function SearchValue({setOpenSearch, search, setSearch}: {setOpenSearch: (type: boolean) => void, search:string, setSearch: (v:string) => void}) {
  ...
  return (
    ...
    <div className={overlay} onClick={() => setOpenSearch(false)}></div>
        <div className={`${animationOpacity} bg-gray-100 w-full p-2 rounded-md absolute top-12 z-90 flex flex-col justify-center items-start gap-2`} onClick={(e) => e.stopPropagation()}>
