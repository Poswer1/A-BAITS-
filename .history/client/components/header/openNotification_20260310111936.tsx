...
export default function OpenNotification() {
  ...
  return (
    <div className='flex flex-col justify-start items-start bg-gray-100 p-2  absolute top-[100%] left-0 rounded-md'>
        <h1>{t('header', 'notification')}</h1>
        {notification.map((n) => (
            <span className='flex flex-wrap w-20'>{t('header', n.notification)} <span className='text-orange-600'>{n?.lot?.name}</span></span>
        ))}
    </div>
  )
}
