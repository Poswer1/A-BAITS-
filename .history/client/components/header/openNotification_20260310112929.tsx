...
export default function OpenNotification({setOpen}: OpenNotification) {
    ...
    <div ref={modalRef} className='flex flex-col justify-start items-start bg-gray-100 p-2 w-100 absolute top-[100%] left-0 rounded-md gap-2 z-30'>
        {loading ? (
            ...
            <>
                ...
                {notification.map((n) => (
                    <span className='flex flex-wrap break-words'>{t('header', n.notification)} <span className='text-orange-600 break-words'>{n?.lot?.name}</span></span>
                ))}
            </>
        )}
    </div>
