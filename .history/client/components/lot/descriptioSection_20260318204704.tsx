  ...
  return (
    <div className={`${columnBlock}`}>
        <div className='flex flex-col justify-start items-start gap-2 max-h-[230px]'>
            <h1 onClick={() => setOpen(prev => !prev)} className='font-bold flex gap-1'>{t('lot', 'desc')}<ChevronDown className={arrowActive(open)}/></h1>
            <div className={`overflow-auto custom-scrollbar ${open ? 'h-full opacity-100' : 'h-0 opacity-0'} transition-all duration-300`}>
