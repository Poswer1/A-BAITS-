...
export default function HeaderLot({lot}:HeaderLot) {
    ...
    const transleteCategory = getValueByLang(categoriesWithIcons, lot.category, lang)
    const transleteSubCategory = getValueByLang(categoriesWithIcons, lot.category, lang)
    const transleteSubSubCategory = getValueByLang(categoriesWithIcons, lot.category, lang)
        ...
        <div className='w-full 2xl:w-[80%] lg:w-[90%] bg-white p-2 flex justify-between items-end'>
            <div className='flex flex-col justify-center items-start gap-2 hidden md:block'>
                ...
                <div className="flex justify-start items-center gap-6 text-gray-800">
                    ...
                    <span className="flex text-black">{lot.category} | {lot.subCategory} {lot.subSubCategory && lot.subSubCategory}</span>
                </div>
            </div>
