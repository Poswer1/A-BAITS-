...
export default function HeaderLot({lot}:HeaderLot) {
    ...
    const TransleteSubCategory = listCategory?.subcategories.find(s => s.name === lot.subCategory)
    const TransleteSubSubCategory = listCategory
        ...
        <div className='w-full 2xl:w-[80%] lg:w-[90%] bg-white p-2 flex justify-between items-end'>
            <div className='flex flex-col justify-center items-start gap-2 hidden md:block'>
                ...
                <div className="flex justify-start items-center gap-6 text-gray-800">
                    ...
                    <span className="flex text-black">{listCategory && lang === 'ru' ? listCategory.ru : listCategory?.uk} | {listSubCategory && lang === 'ru' ? listSubCategory.ru : listSubCategory?.uk} | {listSubCategory?.subcategories && lang === 'ru' ? listSubCategory?.subcategories.ru : listSubCategory?.subcategories?.uk}</span>
                </div>
            </div>
