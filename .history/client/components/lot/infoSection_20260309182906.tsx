...
export default function InfoSection({lot, socket, currentPrice, setCurrentPrice, value, setValue, setStatus,status}: InfoSectionProps) {
    ...
    const handleBuyNow = async () => {
        ...
        if(!token || !lot) return
        try {
            await buyNow(token, lot.lotNumber, value)
            setStatus('Сompleted')
        } catch (error:any) {
  ...
  return ( 
    <div className={`${columnBlock} w-full`}>
        {status !== 'Active' ? (
            <h1 className="font-bold text-3xl">Лот куплен</h1>
        ): (
            <h1 className="font-bold text-gray-500">{t('lot','lot-currentPrice')}<br/><span className="text-3xl">{currentPrice} ₴</span></h1>
        )}
        ...
        {status === 'Active' && (
            <>
                ...
                <button onClick={handleBuyNow} className={`w-full ${hover} text-black border-orange-600 border !text-orange-600 rounded-md p-2 text-lg`}>
