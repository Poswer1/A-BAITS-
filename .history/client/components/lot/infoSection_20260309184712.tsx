...
export default function InfoSection({lot, socket, currentPrice, setCurrentPrice, value, setValue, setStatus,status}: InfoSectionProps) {
    ...
    const handleBuyNow = async () => {
        ...
        if(!token || !lot) return
        try {
            timeRef = await setTimeout(() => {
                setProgress(false)
                buyNow(token, lot.lotNumber, value)
                setStatus('Сompleted')
                setProgress(true)
            }, 5000)
        } catch (error:any) {
    ...
    <div className={`${columnBlock} w-full`}>
        ...
        {status === 'Active' && (
            <>
                ...
                <div className={`${progress ? 'bg-orange-600 transition-all duration-300 ease-in-out' : 'transition-none'} bg-gray-300 h-2 w-10`}></div>
                <button onMouseDown={handleBuyNow} onMouseLeave={handleLeaveBuyNow} onMouseUp={handleLeaveBuyNow} className={`w-full ${hover} text-black border-orange-600 border !text-orange-600 rounded-md p-2 text-lg`}>
