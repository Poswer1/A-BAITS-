    ...
    <div className={`${columnBlock} w-full`}>
        ...
        {status === 'Active' && (
            <>
                ...
                <button onMouseDown={handleBuyNow} onMouseLeave={handleLeaveBuyNow} onMouseUp={handleLeaveBuyNow} className={`w-full ${hover} text-black border-orange-600 border !text-orange-600 rounded-md p-2 text-lg re`}>
                    {t('lot', 'lot-buyNow')} <span className="font-bold">( {lot.blitzPrice} ₴ )</span>
                    <div className={`absolute top-0 left-0 h-full bg-orange-600/10 ${progress ? 'w-full transition-all duration-[5000ms]' : 'w-0'}`}/>
                </button>
            </>
        )} 
    </div>
