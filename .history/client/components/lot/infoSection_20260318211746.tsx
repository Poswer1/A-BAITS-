...
export default function InfoSection({lot, socket, currentPrice, setCurrentPrice, value, setValue, setStatus, status}: InfoSectionProps) {
  ...
  return ( 
    <div onClick={() => setOpen(prev => !prev)} className={`${columnBlock} rounded-t-2xl w-full text-black fixed md:static bg-white z-100 bottom-0 ${open ? '!h-2/3 md:!h-full' : '!h-30 md:!h-full'} ${animate}`}>
        {status !== 'Active' ? (
