...
export default function InfoSection({lot, socket, currentPrice, setCurrentPrice, value, setValue, setStatus, status}: InfoSectionProps) {
  ...
  return ( 
    <div onClick={() => setOpen(prev => !prev)} className={`${columnBlock} rounded-t-2xl w-full text-black fixed md:static bg-white z-100 bottom-0 ${open ? '!h-80 md:!h-full' : '!h-35 md:!h-full'} ${animate}`}>
        {status !== 'Active' ? (
            ...
            <div className="flex justify-between items-center w-full">
                ...
                <button onClick={handleBid} className={`${button} ${animate} ${open ? 'opacity-0 pointer-events-none' : 'opacity-100 pointer-events-none'} text-sm`}>{t('lot', 'lot-doBid')}</button>
            </div>
        )}
