    ...
    <div {...handlers} className={`${columnBlock} rounded-t-2xl w-full text-black fixed md:static bg-white z-20 md:z-0 bottom-0 ${open ? '!h-80 md:!h-full' : '!h-35 md:!h-full'} ${animate}`}>
        ...
        {status === 'Active' && (
            <>
                ...
                <button onMouseDown={handleBuyNow} onMouseLeave={handleLeaveBuyNow} onMouseUp={handleLeaveBuyNow} onTouchStart={handleBuyNow} onTouchEnd={handleLeaveBuyNow}
  onTouchCancel={handleLeaveBuyNow className={`w-full ${hover} text-black border-orange-600 border !text-orange-600 rounded-md p-2 text-lg relative`}>
