...
export default function InfoSection({lot, socket, currentPrice, setCurrentPrice, value, setValue, setStatus, status, auth}: InfoSectionProps) {
  ...
  return ( 
    <div {...handlers} className={`${columnBlock} rounded-t-2xl w-full md:w-50 text-black fixed md:static bg-white z-20 md:z-0 bottom-0 ${open ? '!h-80 md:!h-full' : '!h-35 md:!h-full'} ${animate}`}>
        {status !== 'Active' ? (
