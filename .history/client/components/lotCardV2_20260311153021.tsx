  ...
  return (
    <Link href={`/${lang}/lot/${lot.lotNumber}`} className={`cursor-pointer flex justify-start items-center gap-10 shadow-lg bg-white w-full`}>
        ...
        <div className="flex justify-between items-start w-full">
            ...
            <div className={columnClass}>
                <span>{t('lot', 'lot-state')}: {lot.state}</span>
                <span>{t('lot', 'lot-location')}: {lot.location}</span>
            </div>
            <div className={columnClass}>
                <span>{t('lot', 'lot-dateStop')} 12д 10ч 32м 02с</span>
            </div>
            <div className={columnClass}>
                <span>{t('lot', 'lot-current-bid')}: <span className="text-orange-600 font-bold">{lot.startPrice}₴ </span></span>
                <button className={button}>{t('lot', 'lot-doBid')}</button>
                <span className="flex justify-center items-center gap-2 text-ora"><Star /> {t('lot', 'lot-favorite')}</span>
            </div>
        </div>
    </Link>
  )
