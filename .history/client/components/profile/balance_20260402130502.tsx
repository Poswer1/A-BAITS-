  ...
  return (
     <div className={`${blockClass} !bg-transparent !p-0` }>
      <div className='p-3 bg-white rounded-lg flex justify-between items-center w-full'>
        <h1 className='text-gray-500'>{t('admin', 'CurrentBalance')}: <br/><span className='text-black text-xl'>50 ₴</span></h1>
        <button onClick={() => setOpenQe(true)} className={`${button}`}><DollarSign />{t('admin', 'replenishBalance')}</button>
      </div>
      <h1></h1>
      {openQr && (
