     ...
     <div className={`flex flex-col w-full gap-5` }>
      ...
      {openQr && (
        <div className={overlay} onClick={() => setOpenQe(false)}>
          <div className={`${animationScale} flex flex-col justify-center items-center p-10 w-1/3 bg-white rounded-xl`} onClick={(e) => e.stopPropagation()}>
          <h1>Переведите сумму пополнения по QR-коду или по номеру карты</h1>
            <div className='p-5 bg-gray-100 rounded-xl shadow-md'>
              <QRCode value={qrValue} size={200}/>
            </div>
          <p>Деньги поступят на баланс в течении 24 часов</p>
          </div>
        </div>
      )}
    </div>
