  ...
  return (
     <div className={`flex flex-col w-full gap-5` }>
      <div className='flex flex-col md:flex-row p-3 bg-white rounded-lg justify-between items-center w-full gap-2'>
        ...
        <button onClick={() => setOpenQe(true)} className={`${button} w-full`}><DollarSign />{t('profile', 'replenishBalance')}</button>
      </div>
      <h1 className='text-xl 2xl:text-2xl md:text-xl p-2 py-4 md:p-0 md:mb-2'>{t('profile', 'transactions')}</h1>
