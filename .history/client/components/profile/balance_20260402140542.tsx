     ...
     <div className={`flex flex-col w-full gap-5` }>
      ...
      {openQr && (
        <div className={overlay} onClick={() => setOpenQe(false)}>
          <div className={`${animationScale} flex flex-col justify-center items-center p-10 w-full 2xl:w-1/3 bg-white rounded-xl gap-2`} onClick={(e) => e.stopPropagation()}>
          ...
          <p className='text-gray-500 text-sm md:text-base'>{t('profile', 'balanceDescriptionAboutMoney')}</p>
          </div>
        </div>
      )}
    </div>
