  ...
  return (
    <div className={`flex flex-col justify-center items-center gap-2 bg-white`}>
        {allChats.map((chat) => {
            ...
            if (!user) return null;
            return (
                <div onClick={() => {setSelectChat(user._id), setTypeChat(chat.type)}} className={`${hoverCat} flex justify-between items-start gap-2 cursor-pointer p-3 transition-all duration-300  border-b xl:w-80 2xl:w-90 border-gray-200 `}>
