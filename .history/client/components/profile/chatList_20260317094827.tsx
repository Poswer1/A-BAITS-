  ...
  return (
    <div className={`flex flex-col justify-center items-center gap-2 bg-white`}>
        {allChats.map((chat) => {
            ...
            if (!user) return null;
            return (
                <div onClick={() => {setSelectChat(user._id), setTypeChat(chat.type)}} className={`${hoverCat} flex justify-between items-start gap-2 cursor-pointer p-3 transition-all  duration-300  border-b lg:w-70 2xl:w-90 border-gray-200 `}>
                    <div className='flex justify-center items-center gap-2'>
                        <AvatarBlock avatar={user?.avatar} size="40"/>
                        <div className={`flex flex-col  justify-center items-start `}>
                            <h1>{user.name}</h1>
                            <p className='text-gray-500 text-xs'>Новое сообщение nfrjt!</p>
                        </div>
                    </div>
