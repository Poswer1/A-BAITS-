...
export default function ChatList({setSelectChat, setTypeChat, selectChat}: ChatListProps) {
  ...
  return (
    <div className={`flex flex-col justify-center items-center gap-2 bg-white 2xl:w-90`}>
        {allChats.map((chat) => {
            if(!myId) return null;
            const user = chat.userTo?._id.toString() === myId ? chat.userFrom : chat.userTo;
            if (!user) return null;
            return (
                <div onClick={() => {setSelectChat(user._id), setTypeChat(chat.type)}} className={`${hoverCat} flex justify-between items-start gap-2 cursor-pointer p-3 transition-all  duration-300  border-b  border-gray-200 `}>
