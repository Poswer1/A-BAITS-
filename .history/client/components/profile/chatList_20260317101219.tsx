  ...
  return (
    <div className={`flex flex-col justify-center items-start gap-2 bg-white 2xl:w-90`}>
        {allChats.map((chat) => {
            ...
            if (!user) return null;
            return (
                <div onClick={() => {setSelectChat(user._id), setTypeChat(chat.type)}} className={`${hoverCat} flex justify-between items-start gap-2 cursor-pointer  transition-all  duration-300  border-b  border-gray-200 `}>
