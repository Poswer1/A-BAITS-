...
export default function ChatList({setSelectChat, setTypeChat, selectChat}: ChatListProps) {
  ...
  return (
    <div className={`flex flex-col justify-center items-center gap-2 bg-white w-full md:w-auto`}>
        {allChats.map((chat) => {
            if(!myId) return null;
            const user = chat.userTo?._id.toString() === myId ? chat.userFrom : chat.userTo;
            if (!user) return null;
            return (
                <div onClick={() => {setSelectChat(user._id), setTypeChat(chat.type)}} className={`${hoverCat} flex justify-between items-start gap-2 cursor-pointer p-3 transition-all  duration-300  border-b 2 2xl:w-90 border-gray-200 `}>
                    ...
                    <span className={`text-gray-500 text-sm hidden md:`}>3 часа назад</span>
                </div>
            )
        }
        )}
