  ...
  return (
    <div className={`flex flex-col justify-start items-center gap-2 bg-white w-full md:w-auto`}>
        {allChats.map((chat) => {
            ...
            if (!user) return null;
            return (
                <div onClick={() => {setSelectChat(user._id), setTypeChat(chat.type)}} className={`${hoverCat} flex justify-between items-start gap-2 cursor-pointer p-3 transition-all  duration-300  border-b 2 w-full md:w-auto 2xl:w-90 border-gray-200 relative `}>
                    ...
                    <span className={`text-gray-500 text-sm absolute right-0 md:block`}>3 часа назад</span>
                </div>
            )
        }
        )}
