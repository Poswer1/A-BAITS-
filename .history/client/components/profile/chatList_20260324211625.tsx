...
export default function ChatList({setSelectChat, setTypeChat, selectChat}: ChatListProps) {
  ...
  return (
    <div className={`flex flex-col justify-start items-start w-full md:w-auto gap-2`}>
        <h1 className='mb-1'>{t('chat', 'UnreadChats')}</h1>
        ...
        <h1 className='mb-1'>{t('chat', 'ReadChats')}</h1>
        {readChats.length === 0 ? (
        <span className='text-gray-500'>Нету чатов</span>
        ) : (
