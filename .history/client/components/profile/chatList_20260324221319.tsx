...
export default function ChatList({setSelectChat, setTypeChat, selectChat}: ChatListProps) {
    ...
    const unReadChats = allChats.filter(chat => chat.messages.some(m => m.to && !m.read))
    const readChats = allChats.filter(chat => chat.messages.every(m => m.to && m.read)) // every все ли элементы массива соответствуют условию
  ...
  return (
    <div className={`flex flex-col justify-start items-start w-full md:w-auto`}>
        ...
        <span className='text-gray-500 text-sm'{t('chat', '')}</span>
        ) : (
        ...
        {readChats.length === 0 ? (
        <span className='text-gray-500 text-sm'>Нету чатов</span>
        ) : (
