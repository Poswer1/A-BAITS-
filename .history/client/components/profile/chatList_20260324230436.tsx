...
export default function ChatList({setSelectChat, setTypeChat, selectChat}: ChatListProps) {
    ...
    return chats?.map((chat) => {
        // проверяем, что есть оба пользователя
        if (!chat.userFrom && !chat.userTo) return null;

        // определяем "собеседника" — того, кто не я
        const user = chat.userTo?._id?.toString() === myId
            ? chat.userFrom
            : chat.userTo;
        ...
        return (
            <div
                ...
                onClick={() => {
                    ...
                    setTypeChat(chat.type);
                }}
                className={`${hoverCat} flex justify-between items-start gap-2 cursor-pointer p-3 transition-all duration-300 border-b 2 w-full md:w-auto 2xl:w-90 border-gray-200 bg-white`}
            >
                <div className='flex justify-center items-center gap-2'>
                    <AvatarBlock
                        avatar={chat.lot?.images?.[0] || user?.avatar}
                        size="50"
                    />
                    <div className='flex flex-col justify-center items-start'>
                        ...
                        <p className='text-gray-500 text-sm'>
                            {lastMessage?.message || t('chat', 'NoLatestMessages')}
                        </p>
                    </div>
                </div>
            </div>
