...
export default function ChatList({setSelectChat, setTypeChat, selectChat}: ChatListProps) {
    ...
    return chats?.map((chat) => {
            ...
            return (
                <div onClick={() => {setSelectChat(user._id), setTypeChat(chat.type)}} className={`${hoverCat} flex justify-between items-start gap-2 cursor-pointer p-3 transition-all  duration-300  border-b border-t 2 w-full md:w-auto 2xl:w-90 border-gray-300 bg-white`}>
  ...
  return (
    <div className={`flex flex-col justify-start items-start w-full md:w-auto bg-white p-2`}>
        <h1 className='mb-1 px-2 md:p-0'>{t('chat', 'UnreadChats')}</h1>
        {unReadChats?.length === 0 ? (
        <span className='text-gray-500 text-sm py-4 w-full text-start'>{t('chat', 'AllChatsBeenRead')}</span>
        ) : (
