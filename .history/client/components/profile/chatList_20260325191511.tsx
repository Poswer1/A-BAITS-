...
export default function ChatList({setSelectChat, setTypeChat, selectChat, setLot}: ChatListProps) {
    ...
    return chats?.map((chat) => {
            ...
            return (
                <div onClick={() => {setSelectChat(user._id), setTypeChat(chat.type), setLot(chat._id)}} className={`${hoverCat} flex justify-between items-start gap-2 cursor-pointer p-3 transition-all  duration-300  border-b border-t 2 w-full  2xl:w-90 border-gray-300 bg-white relative`}>
