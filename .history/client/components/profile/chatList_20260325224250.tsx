...
export default function ChatList({setSelectChat, setTypeChat, selectChat, setLot}: ChatListProps) {
  ...
  return (
    <div className={`flex flex-col justify-start items-start w-full md:w-auto  md:min-w-80`}>
        <h1 className='mb-1 px-2'>{t('chat', 'UnreadChats')}</h1>
        <div className='flex w-full overflow-y-auto max-h-50'>
            {loading ? (
            renderSkeleton()
            ): (
        ...
        <div className='flex flex-col w-full overflow-y-auto max-h-50'>
            {loading ? (
            renderSkeleton()
            ): (
            readChats?.length === 0 ? (
