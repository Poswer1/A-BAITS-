import { useTranslation } from "@/app/context/TranslationProvider"
import { loadingBlock } from "@/styles/global"
import { lotListClass, pageContainerClass } from "@/styles/profile/profile"
import Loading from "../utils/loadig"
import LotCard from "../lotCard"
import Sidebar from "./sidebar"

interface LotActivityProps {
    filterLots:any
    mode:string
    slug:string
}

export default function LotActivity({filterLots, mode, slug}: LotActivityProps) {

    const {t} = useTranslation()

  return (
    <div className={pageContainerClass}> 
        <h1 className="text-2xl mb-5">{slug === 'buy' ? t('profile', 'buy') : t('profile', 'sell')} | {active === 'active' ? t('profile', 'active') : active === 'archive' ? t('profile', 'archived') : active === 'completed' ? t('profile', 'completed') : t('profile', 'sold')} {t('global', 'lot')}</h1>
        <div className="w-full flex justify-start items-start gap-5">
          <Sidebar mode={mode} />
          <div className={lotListClass}>
            {filterLots.map((lot:any) => (
              <LotCard lot={lot}/>  
            ))}
          </div>
        </div>
    </div>
  )
}

