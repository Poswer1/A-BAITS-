...
import { Archive, CheckCircle2, Edit2, RotateCcw, Trash2, X, XCircle } from "lucide-react"
import { hover } from "@/styles/style"
import { useEffect, useState } from "react"
import { closeLot, completeLot, deleteLot, resumeLot } from "@/services/lot"
...
export default function LotActivity({data, mode, slug}: LotActivityProps) {
    ...
    const handleAction = () => {
      if(openConfirmWindow === 'delete') {
        handleDeleteLot()
      } else if(openConfirmWindow === 'complete') {
        handleCompleteLot()
              ...
              {(mode === 'sell' && allLots.length > 0 && (active === t('profile', 'active') || active === t('profile', 'completed'))) && (
                <div className={`${openConfirmWindow ? 'w-full md:w-auto': 'w-auto'} flex justify-between items-center gap-2`}>
                  {openConfirmWindow ? (
                    ...
                    {active === t('profile', 'active') && (
                      <span onClick={() => setOpenConfirmWindow('complete')} className={`${styleButtonAction} bg-orange-600 text-white`}><CheckCircle2 size={20}/></span>
                    )}
                    </>
                  )}
                </div>
