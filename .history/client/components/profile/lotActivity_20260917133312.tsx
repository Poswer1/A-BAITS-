...
import { Archive, CheckCircle2, Delete, DeleteIcon, Edit2, RotateCcw, X, XCircle } from "lucide-react"
              ...
              {(mode === 'sell' && allLots.length > 0 && (active === t('profile', 'active') || active === t('profile', 'completed'))) && (
                <div className={`${openConfirmWindow ? 'w-full md:w-auto': 'w-auto'} flex justify-between items-center gap-2`}>
                  {openConfirmWindow ? (
                    ...
                    {active === t('profile', 'active') && (
                      <span onClick={() => setOpenConfirmWindow('complete')} className={`${styleButtonAction} bg-orange-600 text-white`}><DeleteIcon size={20}/></span>
                    )}
                    </>
                  )}
                </div>
