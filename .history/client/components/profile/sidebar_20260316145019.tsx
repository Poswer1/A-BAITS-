...
export default function Sidebar({mode, active, setActive, name} : SidebarProps) {
  ...
  return (
    <div className={`bg-white h-full flex flex-col justify-start items-start min-w-70 ${mode !== 'sidebarMain' && '!h-auto'}`}>
      {listLinks.map(link => (
            <Link
              key={link.link}
              href={`/${lang}/profile/${link.link}`}
              className={`${hoverCat} lg:px-5 2xl:px-10 py-4 border-r-2 border-transparent flex justify-start items-center w-full gap-2 ${active === link.name && linkActiveClass}`}
            >
              {link.icon}
              {link.name}
            </Link>
