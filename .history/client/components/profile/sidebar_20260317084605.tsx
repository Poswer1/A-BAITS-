    ...
    <div className={`bg-white h-full flex ${mode === 'sidebarMain' && 'flex-col'} justify-start items-start lg:min-w-60 2xl:min-w-70 ${mode !== 'sidebarMain' && '!h-auto'}`}>
      {listLinks.map(link => (
            <Link
              ...
              className={`${hoverCat} lg:px-7 2xl:px-10 py-4 border-r-2 border-transparent flex justify-start items-center w-full gap-2 ${active === 'Чат' &&} ${active === link.name && linkActiveClass}`}
            >
              {link.icon}
              {link.name}
            </Link>
