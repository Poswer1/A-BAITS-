...
export default function Sidebar({mode, active, setActive, name} : SidebarProps) {
  ...
  return (
    <div className={`bg-white h-full flex ${mode === 'sidebarMain' && 'flex-col'} justify-start items-start 2xl:min-w-70 ${active === 'Чат' ? 'lg:min-w-10' : 'lg:min-w-60'} transition-all du ${mode !== 'sidebarMain' && '!h-auto'}`}>
      {listLinks.map(link => (
            <Link
              key={link.link}
