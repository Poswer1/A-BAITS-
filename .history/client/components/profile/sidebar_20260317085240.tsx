...
export default function Sidebar({mode, active, setActive, name} : SidebarProps) {
  ...
  return (
    <div className={`bg-white h-full flex ${mode === 'sidebarMain' && 'flex-col'} justify-start items-start 2xl:w-70 ${(active === 'Чат' && mode === 'sidebarMain')  ? 'lg:min-w-10' : 'lg:min-w-60'} transition-all duration-500 ${mode !== 'sidebarMain' && '!h-auto'}`}>
      {listLinks.map(link => (
            <Link
              key={link.link}
