...
export default function Sidebar({mode, active, setActive, name} : SidebarProps) {
  ...
  return (
    <div className={`bg-white h-full flex ${mode === 'sidebarMain' && 'flex-col'} justify-start items-start 2xl:w-70 ${(active === 'Чат' && mode === 'sidebarMain')  ? 'lg:w-20' : 'lg:w-60'} transition-all duration-500 ease-in-out overflow-hidden ${mode !== 'sidebarMain' && '!h-auto'}`}>
      {listLinks.map(link => (
            <Link
              key={link.link}
            ...
            >
              {link.icon}
              <span className={`${(active === 'Чат' && mode === 'sidebarMain') ? 'lg:opacity-0 -translate-x-2 pointer-events-none' : 'lg:block opacity-100'} 2xl:block transition-all duration-3300 ease-in-out `}>{link.name}</span>
            </Link>
        ))}
    </div>
  )
