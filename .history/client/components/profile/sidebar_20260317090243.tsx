...
export default function Sidebar({mode, active, setActive, name} : SidebarProps) {
  ...
  return (
    <div className={`bg-white h-full flex ${mode === 'sidebarMain' && 'flex-col'} justify-start items-start lg: 2xl:min-w-70 ${(active === 'Чат' && mode === 'sidebarMain')  ? 'lg:w-20' : 'lg:w-60'} transition-all duration-500 ease-in-out overflow-hidden ${mode !== 'sidebarMain' && '!h-auto'}`}>
      {listLinks.map(link => (
            <Link
              key={link.link}
