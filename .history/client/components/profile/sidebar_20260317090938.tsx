...
export default function Sidebar({mode, active, setActive, name} : SidebarProps) {
  ...
  return (
    <div className={`bg-white h-full flex ${mode === 'sidebarMain' && 'flex-col'} justify-start items-start 2xl:min-w-70 ${(active === 'Чат' && mode === 'sidebarMain')  ? 'lg:min-w-20' : 'lg:min-w-60'} transition-all duration-500 ease-in-out overflow-hidden will-change-[width] ${mode !== 'sidebarMain' && '!h-auto'}`}>
      {listLinks.map(link => (
            <Link
              key={link.link}
            ...
            >
              ...
              <span className={`${(active === 'Чат' && mode === 'sidebarMain') ? 'lg:opacity-0 absolute' : 'lg:opacity-100 transition-all duration-300 ease-in-out '} 2xl:opacity-100 2xl:static`}>{link.name}</span>
            </Link>
        ))}
    </div>
  )
