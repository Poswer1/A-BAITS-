...
export default function Sidebar({mode, active, setActive, name} : SidebarProps) {
  ...
  return (
    <div className={`bg-white h-full flex ${mode === 'sidebarMain' && 'flex-col'} justify-start items-start 2xl:min-w-70 ${(active === 'Чат' && mode === 'sidebarMain')  ? 'xl:min-w-20' : 'xl:min-w-60'} transition-all duration-500 ease-in-out overflow-hidden will-change-[width] ${mode !== 'sidebarMain' && '!h-auto'}`}>
      {listLinks.map(link => (
            <Link
              key={link.link}
