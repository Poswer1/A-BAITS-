...
export default function Sidebar({mode, active, setActive, name} : SidebarProps) {
  ...
  return (
    <div className={`bg-white h-full flex ${mode === 'sidebarMain' && 'flex-col'} justify-start items-start ${(active === 'Чат' && mode !== 'sidebarMain')  ? 'lg:w-20 2xl:w-20' : 'lg:w-60 2xl:w-70'} transition-all duration-500 ease-in-out overflow-hidden will-change-[width] ${mode !== 'sidebarMain' && '!h-auto'}`}>
      {listLinks.map(link => (
            <Link
              key={link.link}
