...
export default function Sidebar({mode, active, setActive, name} : SidebarProps) {
  ...
  return (
    <div className={`bg-white h-full flex ${mode === 'sidebarMain' && 'flex-col'} justify-start items-start  ${(active === 'Чат' && mode === 'sidebarMain')
      ? 'lg:w-20 lg:min-w-20 2xl:w-24 2xl:min-w-24'
      : 'lg:w-60 lg:min-w-60 2xl:w-72 2xl:min-w-72'} transition-[width] duration-500 ease-in-out overflow-hidden ${mode !== 'sidebarMain' && '!h-auto'}`}>
      {listLinks.map(link => (
            <Link
              key={link.link}
            ...
            >
              ...
              <span className={`${(active === 'Чат' && mode === 'sidebarMain') ? 'opacity-0 absolute' : 'opacity-100 transition-all duration-300 ease-in-out'}`}>{link.name}</span>
            </Link>
        ))}
    </div>
  )
