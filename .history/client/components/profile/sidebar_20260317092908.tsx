...
export default function Sidebar({mode, active, setActive, name} : SidebarProps) {
  ...
  return (
    <div 
    className={`bg-white h-full flex ${mode === 'sidebarMain' && 'flex-col'} 
    justify-start items-start ${mode === 'sidebarMain'? (active === 'Чат'? 'lg:w-20 2xl:w-25': 'lg:w-60 2xl:w-70'): ''} transition-[width] duration-500 ease-in-out overflow-hidden ${mode !== 'sidebarMain' && '!h-auto'}`}>
      {listLinks.map(link => (
            <Link
              key={link.link}
