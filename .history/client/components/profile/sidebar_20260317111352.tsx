...
export default function Sidebar({mode, active, setActive, name} : SidebarProps) {
  ...
  return (
    <div 
      className={`bg-white text-black w-full overflow-x-auto md:h-full flex ${mode === 'sidebarMain' && 'md:flex-col'} 
    ...
    >
      {listLinks.map(link => (
            <Link
              ...
              className={`${hoverCat} px-7 lg:px-7 2xl:px-10 py-4 border-r-2 border-transparent flex justify-start items-center w-full gap-2 ${active === link.name && linkActiveClass}`}
            >
