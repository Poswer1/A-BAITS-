...
export default function Sidebar({mode, active, name} : SidebarProps) {
  ...
  useEffect(() => {
    ...
    if (pathname.includes('buy')) {
      active = t('profile', 'buy')
    } else if (pathname.includes('sell')) {
      active = t('profile', 'sell')
    } else if (pathname.includes('chat')) {
      active = 'Чат'
    } else if (pathname.includes('balance')) {
      active = 'Баланс'
    } else if (pathname.includes('settings')) {
      active = t('profile', 'setting')
    } else if (pathname.includes('reviews')) {
      active = t('profile', 'Reviews')
    } else if (pathname.includes('lots')) {
      active = t('global', 'lot')
    } else {
      active = t('profile', 'profile')
    }

    setActiveLink(active) // одно состояние для активной ссылки
  }, [pathname, t])
      ...
      {listLinks.map(link => {
        return (
          <Link
              ...
              className={`${hoverCat} px-5 lg:px-7 2xl:px-10 py-4 border-r-2 border-transparent flex justify-start items-center w-full gap-2 ${activeLink === link.name && linkActiveClass}`}
            >
              <div className="transition-all duration-300">{link.icon}</div>
              <span className={`${(activeLink === 'Чат' && mode === 'sidebarMain') ? 'md:opacity-0 md:absolute' : 'opacity-100 transition-all duration-300 ease-in-out'}`}>{link.name}</span>
            </Link>
        )
        })}
    </div>
