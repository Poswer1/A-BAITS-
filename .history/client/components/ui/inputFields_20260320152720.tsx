...
export default function InputField({
  ...
  return (
    <div className={`flex flex-col justify-start items-start w-full`}>
      ...
      <div className={`flex justify-start items-center w-full border border-gray-100 bg-gray-100 ${maxLength && stringValue.length > maxLength ? 'border-red-600 bg-red-600/10' : ''} rounded-md `}>
        {textarea ? (
          <textarea
      ...
      {maxLength && (
        <div className={`flex justify-between items-center w-full`}>
          <span className={`text-gray-500 text-sm ${stringValue.length > maxLength && 'text-red-600'}`}>{maxLength === 70 ? t('createLot', 'createLot-lengthName') : 'createLot-lengthdescriptions'}</span>
