      ...
      <span>{title}</span>
        ...
        {open && (
            <div className='fle flex-col justify-start items-start bg-gray-100 w-full rounded-b-md h-50 overflow-auto custom-scrollbar'>
                {list?.map((obg, index) => (
                    <span key={index} onClick={() => {setValue(lang === 'ru' ? obg.ru ||obg.name : obg.uk ||), setOpen(false)}} className={`py-1 px-2 ${hoverCat} w-full flex`}>{lang === 'ru' ? obg.ru : obg.uk || obg.name}</span>
                ))}
            </div>
        )}
    </div>
