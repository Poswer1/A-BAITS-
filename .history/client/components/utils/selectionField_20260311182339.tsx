...
export default function SelectionField({title, placeholder, list, setValue, value}:SelectionFieldProps) {
    ...
    <div className='flex flex-col justify-start items-start w-full'>
      <span>{title}</span>
        ...
        {open && (
            <div className='fle flex-col justify-start items-start bg-gray-100 w-full rounded-b-md min-h-auto max-h-50 overflow-auto custom-scrollbar absolute top-0 left-0'>
                {list?.map((obg, index) => (
