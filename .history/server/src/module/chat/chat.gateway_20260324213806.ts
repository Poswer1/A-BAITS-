...
export class ChatGateway {
    ...
    async readChat(@MessageBody() data:{toUserId:string, type:string}, @ConnectedSocket() client:Socket) {
        const userId = client.data.userId
        if(!userId) return console.log('айди не найден при прочтения сообщений')
    }
