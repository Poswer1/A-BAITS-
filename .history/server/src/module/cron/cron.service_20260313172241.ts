        ...
        try {
            ...
            for (const lot of expiredLots) {
                if (lot.historyBid.length > 0) {
                    ...
                    const newNotification = await this.notificationService.createNotification(lot._id.toString(), winner.author.toString(), lot.author.toString(), 'lotPurchased')
                    this.notificationGateWay.sendNotification(newNotification)
                } else {
                    lot.status = 'Archive'
