...
export class LotController {
  ...
  @Patch('updateLot/:id')
  @UseInterceptors(FilesInterceptor('images', 8, ImagesInterceptor('./uploads/lots')))
  async updateLot(@Body('role') role:string, @Body() dto: LotDto, @Param('id') id:string,  @UploadedFiles() files: Express.Multer.File[], @CurrentUser('id') userId:string) {
    return this.lotService.updateLot(dto, id, files, userId, role)
  }
