...
export class LotService {
  ...
  async getFilterLot(query: filterLot) {
    ...
    if(subSubCategory !== undefined)filter.subSubCategory = subSubCategory
    if(city !== undefined)filter.location = city
    if (min || max) {
      filter.startPrice = {
        ...(min ? { $gte: min } : {}),
        ...(max ? { $lte: max } : {})
    ...
    if (state) {
      const states = Array.isArray(state) ? state : [state]; // если строка → делаем массив
      filter.state = {
        $in: states.map(s => s.toLowerCase())
      }
    }

    return await LotModel.find(filter)
