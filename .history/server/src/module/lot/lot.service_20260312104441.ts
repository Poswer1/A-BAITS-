...
export class LotService {
  ...
  async getFilterLot(query: filterLot) {
    ...
    const {category, subCategory, subSubCategory, city, minPrice, maxPrice, state, sort} = query
    ...
    const sortOption = sort === 'LowToUp' ? { startPrice: 1 } : { startPrice: -1 }
    ...
    const [lots, totalLot] = await Promise.all([
      LotModel.find(filter).sort(sortOption)
      ,
      LotModel.countDocuments(filter)
    ])
    return {lots, totalLot}
