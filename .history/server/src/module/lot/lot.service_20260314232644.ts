...
export class LotService {
  ...
  async getMyLots(query: getMyLotsDto, userId:string) {
    ...
    if(mode === 'sell' ) {
      console.log(`вызвался ${mode}`)
      filter.author = userId
      if(status)filter.status = status
    } else {
      console.log(`вызвался ${mode}`)
      if(status === 'Active'){
        filter['historyBid.author'] = new Types.ObjectId(userId)
      ...
      if(status === 'Completed') {
        filter.winner = new Types.ObjectId(userId)
        filter.status = 'Completed'
      }
      if(status === 'Sold') {
        filter.winner = { $ne: new Types.ObjectId(userId) } // $ne не равняеться
        filter.status = 'Completed'
      }
      if(status === 'Favorite') {
        console.log(status)
        console.log(userId || 'yt')
