  ...
  async createLot(dto: LotDto, files: Express.Multer.File[], userId:string) {
    ...
    const session = await mongoose.startSession()
      try {
        ...
        const [product] = await LotModel.create( // когда делаем [product] говорим то что берем первый элеент массива что бы вернуть обьект а не массив на фронт
          [
            {
              ...dto,
              author: userId,
      ...
      throw new BadRequestException('ErrorCreate',error.message)
    } finally {
      session.endSession()
    }
  }
