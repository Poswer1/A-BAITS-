...
export class LotService {
  ...
  async updateLot(dto: LotDto, id:string, files: Express.Multer.File[], userId:string) {
    const images = files ? await ProccessImages(files, '/uploads/lots/') : []
    ...
    if(dto.date) {
      ...
      date = new Date(nowDate.getTime() + (oneDay * dto.date))
      if(dto.dateTime) {
        const [hours, minutes] = dto.dateTime.split(':').map(Number)
    ...
    if(!updateLot) throw new BadRequestException()
