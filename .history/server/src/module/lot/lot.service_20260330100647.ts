...
export class LotService {
  ...
  async updateLot(dto: LotDto, id:string, files: Express.Multer.File[], userId:string, role:string) {
      ...
      const existingImages = lot.images || [];
    ...
    const updateLot = await LotModel.findOneAndUpdate(
      {
        lotNumber: id,
        author: userId
        ...(updatedImages.length > 0 && { images: existingImages }) // если есть новые изображения, то проверяем наличие существующих изображений
      },
      {
        ...dto,
        date: newDate
