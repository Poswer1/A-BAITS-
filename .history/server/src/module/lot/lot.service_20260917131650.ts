  ...
  async deleteLot(id:string, role:string) {
    ...
    if (role !== 'admin') {
      if (lot.status === 'Archive') {
        throw new BadRequestException('ArchivedLotCannotBeChanged')
      }
      if (lot.status !== 'Active' || (lot.historyBid && lot.historyBid.length > 0)) {
        throw new BadRequestException('LotCannotBeDeleted')
      }

      const completedLot = await LotModel.findByIdAndUpdate(
        id,
        { $set: { status: 'Completed' } },
        { returnDocument: 'after' }
      )
      if (!completedLot) throw new BadRequestException('errorDeleteLot')
      return { success: true, status: completedLot.status }
    }
