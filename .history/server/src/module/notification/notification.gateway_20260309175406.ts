...
export class NotificationGateway {
    ...
    async sendNotification(@MessageBody() data: {lotId:string, to:string, from:string, notification:string}, @ConnectedSocket() client:Server) {
        const {to, from, notification, lotId} = data

        const newNotification = await this.notificationService.createNotification(lotId)
