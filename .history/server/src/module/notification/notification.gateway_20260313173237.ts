...
export class NotificationGateway {
    ...
    async sendNotification(data: {lotId:string, to:string, from?:string, notification:string}) {
        const {to, from, notification, lotId} = data
            if(from) {
                const newNotification = await this.notificationService.createNotification(lotId, '', to, notification)
                for(const id [to, from]) {
                    this.server.to(id).emit('newNotification', newNotification)
                } else {
                    const newNotification = await this.notificationService.createNotification(lotId, 'fro, to, notification)
                }
            }
