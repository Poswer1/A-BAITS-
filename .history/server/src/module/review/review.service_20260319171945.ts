...
export class ReviewService {
    ...
    async newReview(userId:string, dto:reviewDto) {
        ...
        const newMessage = await chat.messages.push({from: new Types.ObjectId('507f1f77bcf86cd799439011'), to: user._id, message: 'NewReview', read:true, createdAt: new Date()})

        await chat.save()

        this.chatGateWay.newReview(user._id.toString(), newMessage)
