...
export const ProccessImages = async (files: Express.Multer.File[], destination:string) => {
    ...
    await Promise.all( // Promise.all ждет пока все промисы завершаться такие как sharp .toFile fs.unlinkSync они возрощают промисы
        files.map(async (file) => {
            const filePath = path.join(process.cwd(), destination.slice(1)) 
            ...
            const webpPath = path.join(filePath, webpName) // file.destination путь к папку куда был загружен файл
            ...
            await fsunlink(file.path) // unlinkSync синхронно удаляет файл с диска.
