    ...
    <div className={`${block} gap-4`}>
        <div className={Blockinput}>
         ...
         <div className={Blockinput}>
            ...
            <div onClick={() => setOpenCategory(prev => !prev)} className={` ${hover} flex justify-between items-center w-full md:w-1/2 bg-gray-100 text-black p-2 rounded-md border-2 ${categoryError ? 'border-red-600 bg-red-600/10' : 'border-transparent'} ${createLotCategory && 'bg-orange-600/10'}`}>
                {createLotCategory ? (
            ...
            {categoryError && <span className='text-red-600 text-sm'>{categoryError}</span>}
        </div>
      </div>
  )
}
