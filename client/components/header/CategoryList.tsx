...
import { getCategoryStats } from '@/services/lot';
...
function CategoryList({setOpenCategory, openFrom, createLotSetCategory, createLotSetSubCategory, createLotSetSubSubCategory} : CategoryList) {
  ...
  const getCategoryCount = (catName: string) => categoryStats[catName]?.count ?? 0
  const getSubCategoryCount = (catName: string, subName: string) => categoryStats[catName]?.subcategories?.[subName]?.count ?? 0
  const getSubSubCategoryCount = (catName: string, subName: string, subSubName: string) => categoryStats[catName]?.subcategories?.[subName]?.subSubcategories?.[subSubName] ?? 0
