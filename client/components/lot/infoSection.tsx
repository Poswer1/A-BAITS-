...
interface InfoSectionProps {
    ...
    auth:boolean
    buyConfirm:boolean
    setBuyConfirm: React.Dispatch<React.SetStateAction<boolean>>
}

export default function InfoSection({lot, socket, currentPrice, setCurrentPrice, value, setValue, setStatus, status, auth, buyConfirm, setBuyConfirm}: InfoSectionProps) {
  ...
  return ( 
    <div {...handlers} className={`${columnBlock} rounded-t-2xl w-full md:min-w-80 text-black fixed md:static bg-white z-20 md:z-0 bottom-0 ${open ? '!h-80 md:!h-full' : '!h-35 md:!h-full'} ${animate}`}>
        ...
        {status === 'Active' && (
            <>
            <span className="text-black flex gap-1 flex-col">{t('lot', 'lot-dateStop')}<Countdown date={lot.date.toString()}/></span>
                    ...
                    <>
                        ...
                        {myAutoBid !== null && (
                            <div className="text-sm text-gray-500 pt-2">
                                {t('lot', 'autoBidMax')}: <span className="font-semibold text-black">{myAutoBid} ₴</span>
                            </div>
                        )}
        ...
        {buyConfirm &&(
            <ModalConfirm 
            handleClose={() => setBuyConfirm(false)}
            handleAction={handleBuyNow}
            title={t('lot', 'confirmBuyNow')}
            alert={''}
            yesButton={t('lot', 'yesBuyNow')}
        />  
        )}
    </div>
  )
