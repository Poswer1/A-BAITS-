...
let ChatService = class ChatService {
    ...
    async inviteAdmin(id) {
        ...
        if (lastRequest && Date.now() - lastRequest < 60_000) {
            throw new common_1.BadRequestException('ExceededLimitPerMinute');
        }
    ...
    async getChatId(myId, userId, lotId) {
        try {
            const chatId = await chat_model_1.ChatModel.findOne({
                users: { $all: [new mongoose_1.Types.ObjectId(myId), new mongoose_1.Types.ObjectId(userId)] },
                lot: lotId
            }).select('_id');
            if (!chatId)
            ...
            console.error(error);
            throw new common_1.BadRequestException('Ошибка при получении id чата');
        }
    }
